...
app.post('/api/convert', upload.single('file'), async (req, res) => {
  try {
    ...
    if (mime === 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' || name.toLowerCase().endsWith('.docx')) {
      markdown = await docxToMarkdown(req.file.buffer);
    } else if (mime === 'application/pdf' || name.toLowerCase().endsWith('.pdf')) {
      markdown = await pdfToMarkdown(req.file.buffer);
    } else if (mime.startsWith('image/')) {
      const base64 = req.file.buffer.toString('base64');
      markdown = await imageToMarkdownWithVision(base64, process.env.GOOGLE_VISION_API_KEY);
    } else {
      return res.status(415).json({ error: `Tipo de arquivo não suportado: ${mime}` });
    }
